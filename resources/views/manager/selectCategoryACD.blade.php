    ...
    <div class="col-sm-3 col-md-3 col-lg-3">
        ...
        <div style="height: 400px; width: 100%; overflow-y: auto;
            ...
            <div style='margin-top: 5px; margin-left: 10px;'>
                ...
                <a href='#' style='font-size: 20px; float:left;' onclick="selectCategoryABD(<?=$item->categorya_id?>, <?=$item->categoryb_id?>, <?=$item->id?>); return false;">
                    <div class='hoverLightBlue' style='border: 1px solid white; width: 100%; min-height: 30px;'>
                        <?=$item->name?>
                    </div>
                </a>
                <a href="#" class="modifyName" style="float:left; margin-left: 5px;" onclick="modifyCategoryC(<?=$item->categorya_id?>, <?=$item->categoryb_id?>, <?=$item->id?>, '<?=$item->name?>'); return false;">
                    <span>수정</span>
                </a>
                <a href="#" class="deleteName" style="float:left; margin-left: 3px;" onclick="deleteCategoryC(<?=$item->categorya_id?>, <?=$item->categoryb_id?>, <?=$item->id?>, '<?=$item->name?>'); return false;">
                    <span>삭제</span>
                </a>
            </div>
    ...
    <div class="col-sm-12 col-md-12 col-lg-12 text-center">
        <div class="inputSection">
            <form action="#" method="post" name="form" id="form">
                ...
                <input type="text" id="txtCategoryC" required />
                <button type="submit" class="buttonC">확인</button>
            </form>
        </div>
    </div>
</div>
                    ...
                    <div class="row">
                        <div class='form-group'>
                            ...
                            <div class='col-sm-offset-1 col-md-offset-1 col-lg-offset-1 col-sm-2 col-md-2 col-lg-2'>
                                <button type='submit' id="create" name="create" class='btn btn-default btn-primary' onclick="updateCategoryC(); return false;">
                                    확인
                                </button>
                            </div>
                        </div>
                ...
                <form name="form" id="form" action="#" method="post" class='form-horizontal' enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class='form-group'>
                            ...
                            <div class='col-sm-offset-1 col-md-offset-1 col-lg-offset-1 col-sm-2 col-md-2 col-lg-2'>
                                <button type='submit' id="create" name="create" class='btn btn-default btn-primary' onclick="deleteUpdateCategoryC(); return false;">
                                    확인
                                </button>
                            </div>
                        </div>
...
<script>
    function modifyCategoryC(id, id2, id3, name)
    {
    ...
    function deleteCategoryC(id, id2, id3, name)
    {
    ...
    function updateCategoryC()
    {
        ...
        $.ajax({
            ...
            url: "/manager/updateCategoryC",
            data: { id: id, id2: id2, id3: id3, name: name },
            chche: false,
            ...
            error: function (data) {
                alert("/manager/updateCategoryC 시스템에러...");
            }
        });
    }

    function deleteUpdateCategoryC()
    {
        ...
        $.ajax({
            ...
            error: function (data) {
                alert("/manager/deleteCategoryC 시스템에러...");
            }
        });
    }
</script>
...
<script>
    $(function () {
        $('#form').submit(function () {
            var name = $('#txtCategoryC').val().trim();
            if (name.length < 1) {
                alert("한글자 이상 입력하세요...");
                return;
            }
            ...
            $.ajax({
                ...
                url: "/manager/insertCategoryC",
                data: { id: idAjax, id2: id2Ajax, txtCategoryC: name },
                chche: false,
                success: function (data) {
                    if (data == 1) {
                        alert("등록 완료...");
                ...
                error: function (data) {
                    alert("/manager/insertCategoryC 시스템에러...");
                }
            });
            return false;
