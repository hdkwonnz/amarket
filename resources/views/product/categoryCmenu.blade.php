    ...
    <div class="col-sm-12 col-md-12 col-lg-12" style="margin-top: 10px;">
        ...
        <div style="float: left">
            ...
            <div class="children_category" style="background: white; z-index: 4; 
                ...
                ?>
                <div>
                    <!--<a href="/products/categoryBmenu?id=<?=$item->categoryAId?>&id2=<?=$item->categoryBId?>&nameB=<?=$item->nameB?>">-->
    ...
    <div class="col-sm-12 col-md-12 col-lg-12">
        <div style="border: 1px solid rgba(0, 38, 255, 0.63); width: 100%; min-height: 50px; overflow: hidden; margin-top: 10px;">
            ...
            foreach ($categoryds as $item)
            {
                ...
                $nameC = $item->cname;
            ?>
            <div class="col-sm-3 col-md-3 col-lg-3" style="padding: 10px;">
                <!--<a href="/products/categoryDmenu?id=<?=$item->categoryAId?>&id2=<?=$item->categoryBId?>&id3=<?=$item->categoryCId?>&id4=<?=$item->categoryDId?>&nameB=<?=$item->nameB?>&nameC=<?=$item->nameC?>&nameD=<?=$item->nameD?>">-->
                                                                                                                           <!--$item->id?-->                       <!--$item->name-->
...
<hr style="margin-bottom: 3px; margin-right: 15px; margin-top: 0px;" />
...
<!--categoryCproducts class 안에 Pagination을 사용하여-->
<!--Ajax로 원하는 데이터를 보여준다-->
<script>
    $(function () {
        getMultiColumn();
    ...
    function getMultiColumn() {
        ...
        $.ajax({
            url: "/product/categoryCAll",
            type: 'Get',
            cache: false,
            data: { id: idAjax, id2: id2Ajax, id3: id3Ajax },
            success: function (data) {
                //$(".categoryCproducts").html(data);
                $(".categoryCproducts").empty().append(data);
            },
            error: function () {
                alert(data + "   /product/categoryCAll something seems wrong...");
            }
        });
